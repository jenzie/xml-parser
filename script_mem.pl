...
sub main {
	...
	# Open results directory and get list of items
	opendir(DIR, "results");
	my @dirs = readdir(DIR);
	closedir(DIR);
	...
	foreach my $dir (@dirs) {
		...
		if(-d "results\\$dir") {
			...
			my $tmppath = getcwd();
			$tmppath = "$tmppath\\results\\$dir";
			print "Processing directory: $tmppath\n";
			...
			print "(3/6) Performing approximation on .xml files.\n";
    			...
    			# Run the .exe
    			monexec("\"output_src\\scimark_v1.exe\" > \"output_src\\scimark_v1.txt\"");
		}
	}
}
