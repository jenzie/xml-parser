...
static void FFT_transform_internal (int N, float *data, int direction, int print) {
    int n = N/2;
    int bit = 0;
    int logn;
    int dual = 1;
    ...
    for (bit = 0; bit < logn; bit++, dual *= 2) {
      ...
      float theta = 2.0 * direction * PI / (2.0 * (float) dual);
      float s = sin(theta);
      float t = sin(theta / 2.0);
      float s2 = 2.0 * t * t;
...
// MODIFIED FOR TEST CODE
void FFT_transform(int N, float *data, int print)
{
  FFT_transform_internal(N, data, -1, print);
}


void FFT_inverse(int N, float *data, int print)
{
    int n = N/2;
    float norm = 0.0;
    int i=0;
