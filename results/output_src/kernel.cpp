...
#include "array.h"
    ...
    float kernel_measureFFT(int N, float mintim, Random R)
    {
        /* initialize FFT data as complex (N real/img pairs) */

        int twoN = 2*N;
        float *x = RandomVector(twoN, R);
        long cycles = 1;
	...
	printf("\n%s\n\n", "Starting FFT");
        ...
        while(1)
        {
	      ...
	      FFT_transform(twoN, x, printResult);     /* forward transform */
	      FFT_inverse(twoN, x, printResult);       /* backward transform */
	      printResult = 0; /*only print result once*/
            }
            Stopwatch_stop(Q);
            if (Stopwatch_read(Q) >= mintim)
                break;
    ...
    float kernel_measureSOR(int N, float min_time, Random R)
    {
        float **G = RandomMatrix(N, N, R);
        float result = 0.0;
	int print = 1;
	printf("\n%s\n", "Starting SOR");
        ...
        {
            Stopwatch_start(Q);
            SOR_execute(N, N, 1.25, G, cycles, print);
	    print = 0;
            Stopwatch_stop(Q);
	...
	printf("%s\n", "Starting MonteCarlo");
        ...
        {
            Stopwatch_start(Q);
            MonteCarlo_integrate(cycles, print);
	    print = 0;
            Stopwatch_stop(Q);
            if (Stopwatch_read(Q) >= min_time) break;
    ...
    float kernel_measureSparseMatMult(int N, int nz, 
            float min_time, Random R)
    {
	...
	printf("%s\n", "Starting SparseRow");
        ...
        {
            Stopwatch_start(Q);
            SparseCompRow_matmult(N, y, val, row, col, x, cycles, print);
	    print = 0;
            Stopwatch_stop(Q);
            if (Stopwatch_read(Q) >= min_time) break;
    ...
    float kernel_measureLU(int N, float min_time, Random R)
    {
